...
2. **ToT (Tree of Thoughts, 思维树)**：CoT 的高阶演进版本。当单一线性思考（CoT）可能走入死胡同时，ToT 允许模型在每一步生成多个可能的分支选项（Thoughts），并利用大模型本身对这些分支进行评估打分（类似启发式搜索），主动进行状态回溯或分支剪枝，从而寻找全局最优解。非常适合极度复杂的规划或推演任务。
3. **Few-Shot (少样本提示)**：在 Prompt 中塞入 2-3 个符合预期输入输出格式的示例，利用模型强大的上下文学习（In-Context Learning）能力，规范最终的输出结构。
4. **约束性输出 (Constrained Decoding)**：在 API 调用层面强约束模型返回 JSON 等结构化数据格式，配合 Regex 或 JSON Schema 解析。
